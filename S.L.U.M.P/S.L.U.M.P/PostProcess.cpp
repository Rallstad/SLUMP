...
bool checkIfValidDate(std::string possibleDate) {
	...
	std::cout << "Expiration Date: " <<std::to_string(day) << std::endl;
	std::cout << "Expiration month: " << std::to_string(month) << std::endl;
    std::cout << "Expiration year: " << std::to_string(year) << std::endl;
    
    int thisDay;
    int thisMonth;
    int thisYear;
    std::cout << "What date is it today?: " << std::endl;
    std::cin >> thisDay;
